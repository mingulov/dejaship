...
The agent uses the density data to decide: proceed, pivot, or contribute to a shipped project via its `resolution_url`.

**B. Claim** — Agent calls `dejaship_claim_intent`. Gets `claim_id` + secret `edit_token` (must be saved — cannot be recovered).
